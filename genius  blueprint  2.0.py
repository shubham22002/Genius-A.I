...
def PDF():
    ...
    if 'marathi' in lan:                            # make pdf read in marathi
        ...
        try:
            ...
            print("Please Wait...")
            ty.save(f"{ask}.mp3")
            os.startfile(book)         # automatically opens the pdf file(Note:if you use Mircosoft Edge)
            time.sleep(3)
...
def Youtube():
    ...
    if 'mute' in orde:                     # this the automation of youtube upto volume down query
        pyautogui.press('m')
    ...
    if 'volume up' in orde:
        pyautogui.hotkey('volumeup')

    if 'volume down' in orde:
        pyautogui.hotkey('volumedown')

def Chrome():
    orde2 = takecommand()

    if 'close this tab' in orde2:  # automation of chrome
        pyautogui.hotkey('ctrl', 'w')

    if 'open new tab' in orde2:
        pyautogui.hotkey('ctrl', 't')

    if 'open new window' in orde2:
        pyautogui.hotkey('ctrl', 'n')

    if 'history' in orde2:
        pyautogui.hotkey('ctrl', 'h')

    if 'download' in orde2:
...
while True:
    ...
    if 'play' in query:                          # speak play before search the song and video also
    ...
    if 'location' in query or 'map' in query:
