...
while True:
    ...
    if 'retype' in query:          # works on google as well as youtube
        pyautogui.press('/')
        pyautogui.hotkey('ctrl', 'a')
        pyautogui.press('delete')

    if 'type' in query:           # works on google as well as youtube (note:Speak retype to clear the text and then speak retype)
        wr = takecommand()
        wr = wr.replace("type", "")
        pyautogui.write(wr)
        pyautogui.press('enter')
